...
class EscolaDAO(context: Context) {
    ...
    fun getById(id: Int): Escola? {
        ...
        val cursor: Cursor = db.rawQuery("SELECT id, nome, endereco FROM ${DatabaseHelper.TBL_ESCOLAS} WHERE id = ?", arrayOf(id.toString()))
    ...
    fun listar(): List<Escola> {
        ...
        val db = dbHelper.readableDatabase
        val cursor: Cursor = db.rawQuery("SELECT id, nome, endereco FROM ${DatabaseHelper.TBL_ESCOLAS}", null)
        if (cursor.moveToFirst()) {
            do {
                lista.add(
                    Escola(
